...
def foo_default_1(x: int = 1) -> int:
    return x


def foo_default_2(x: int = 2) -> int:
    return x


def foo_default_none(x: int = None) -> int:
    return 0 if x is None else x


def foo_default_missing(x: int) -> int:
    return x


def test_param_default_value_changes():
    cases = [
        # Changing the default value is compatible (permissive)
        (foo_default_1, foo_default_2, True),
        (foo_default_2, foo_default_1, True),
        (foo_default_1, foo_default_none, True),
        (foo_default_none, foo_default_1, True),
        # Removing a default is not compatible
        (foo_default_1, foo_default_missing, False),
        (foo_default_missing, foo_default_1, True),  # adding a default is compatible
    ]
    for f1, f2, expected in cases:
        assert is_compatible(f1, f2) == expected, (
            f"{f2.__name__} should {'be' if expected else 'not be'} compatible with {f1.__name__} (default value change)"
        )
